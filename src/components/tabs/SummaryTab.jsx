...
export default function SummaryTab({
  ...
  function getModeHint() {
    ...
    if (mode === "keto") return "Κράτα τους υδατάνθρακες κάτω από 8g/100g.";
  ...
  function getSuggestedFoods() {
    ...
    return foods
      ...
      .sort((a, b) => {
        ...
        const rem = Math.max((proteinTarget||0)-(totalProtein||0),0);
        if (rem > 15) { aScore += aP*3; bScore += bP*3; } else { aScore += aP*1.5; bScore += bP*1.5; }
        if (mode==="high_protein"){aScore+=aP*2;bScore+=bP*2;}
        if (mode==="low_carb"){aScore-=aCr*2;bScore-=bCr*2;}
        if (mode==="keto"){aScore-=aCr*4;bScore-=bCr*4;}
  ...
  return (
    <>
      ...
      {/* 3. AI COACH */}
      <AiCoach last7Days={last7Days} dailyLogs={dailyLogs} targetCalories={targetCalories} proteinTarget={proteinTarget} mode={mode} goalType={goalType} streak={streak} weightLog={weightLog} favoriteFoods={favoriteFoods} foods={foods} totalCalories={totalCalories} totalProtein={totalProtein} exerciseValue={exerciseValue} remainingCalories={remainingCalories} favoriteFoodsText={favoriteFoodsText} favoriteExercisesText={favoriteExercisesText} favoriteExercises={favoriteExercises}/>
      ...
      <div className="card">
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 14px",background:"var(--bg-soft)",borderRadius:12,border:"1px solid var(--border-soft)",marginBottom:16}}>
          ...
          <div style={{fontWeight:800,fontSize:28,lineHeight:1}}>{streak}<span className="muted" style={{fontSize:14,fontWeight:400,marginLeft:4}}>μέρες</span></div>
        </div>
          ...
          <div style={{marginBottom:10,overflowX:"auto"}}>
            <svg viewBox={`0 0 ${chartW} ${chartH+16}`} style={{width:"100%",maxWidth:chartW}}>
              {chartData.map((point,i)=>{
                ...
                return(<g key={point.date}>{next&&<line x1={x} y1={y} x2={nx} y2={ny} stroke="var(--color-accent,#111)" strokeWidth="1.5"/>}<circle cx={x} cy={y} r="3" fill="var(--color-accent,#111)"/>{showLabel&&<text x={x} y={chartH+14} textAnchor="middle" fontSize="7" fill="var(--text-muted,#888)">{point.date.slice(5)}</text>}</g>);
              })}
            </svg>
            {diff!==null&&<div style={{fontSize:12,marginTop:4}}><span className="muted">30 μέρες: </span><strong style={{color:diff<=0?"#22c55e":"#ef4444"}}>{diff>0?"+":""}{Math.round(diff*10)/10} kg</strong>{lastWeight&&<span className="muted"> · Τώρα: {lastWeight} kg</span>}</div>}
          </div>
        )}
        {sortedWeightLog.length>0&&(
          <div style={{borderTop:"1px solid var(--border-soft)",paddingTop:10}}>
            ...
            {sortedWeightLog.length>3&&<button className="btn btn-light" style={{marginTop:8,width:"100%",fontSize:12}} onClick={()=>setShowAllWeight(!showAllWeight)} type="button">{showAllWeight?"Λιγότερα ▲":`+${sortedWeightLog.length-3} ακόμα ▼`}</button>}
          </div>
        )}
      </div>
