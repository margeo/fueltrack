    ...
    <>
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {/* Fitness level */}
          {(() => {
            const isOpen = expandedSections.level;
            return (
              <div>
                <button type="button" onClick={() => setExpandedSections(prev => ({ ...prev, level: !prev.level }))}
                  ...
                  <span>{t("exercisePrefs.fitnessLevel")}</span>
                  <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    {fitnessLevel && <span style={{ background: "var(--color-accent)", color: "var(--bg-card)", borderRadius: 10, padding: "1px 8px", fontSize: 11, fontWeight: 700 }}>{t("exercisePrefs.level." + fitnessLevel)}</span>}
          ...
          {/* Workout location */}
          {(() => {
            const isOpen = expandedSections.location;
            return (
              <div>
                <button type="button" onClick={() => setExpandedSections(prev => ({ ...prev, location: !prev.location }))}
                  style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", borderRadius: 10, border: "1px solid var(--border-color)", background: "var(--bg-soft)", cursor: "pointer", fontSize: 13, fontWeight: 700, color: "var(--text-primary)" }}>
                  <span>🏠 {t("exercisePrefs.workoutLocation")}</span>
                  <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    {workoutLocation && <span style={{ background: "var(--color-accent)", color: "var(--bg-card)", borderRadius: 10, padding: "1px 8px", fontSize: 11, fontWeight: 700 }}>{t("exercisePrefs.location." + workoutLocation)}</span>}
                    <span style={{ fontSize: 11, color: "var(--text-muted)" }}>{isOpen ? "▲" : "▼"}</span>
                  </span>
                </button>
                {isOpen && (
          ...
          {/* Equipment */}
          {(() => {
            const isOpen = expandedSections.equipment;
            const count = equipment.length;
            return (
              <div>
                <button type="button" onClick={() => setExpandedSections(prev => ({ ...prev, equipment: !prev.equipment }))}
                  style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", borderRadius: 10, border: "1px solid var(--border-color)", background: "var(--bg-soft)", cursor: "pointer", fontSize: 13, fontWeight: 700, color: "var(--text-primary)" }}>
          ...
          {/* Frequency & Duration */}
          {(() => {
            const isOpen = expandedSections.schedule;
            return (
              <div>
                <button type="button" onClick={() => setExpandedSections(prev => ({ ...prev, schedule: !prev.schedule }))}
                  style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", borderRadius: 10, border: "1px solid var(--border-color)", background: "var(--bg-soft)", cursor: "pointer", fontSize: 13, fontWeight: 700, color: "var(--text-primary)" }}>
          ...
          {/* Fitness goals */}
          {(() => {
            const isOpen = expandedSections.goals;
            const count = (fitnessGoals || []).length;
            return (
              <div>
                <button type="button" onClick={() => setExpandedSections(prev => ({ ...prev, goals: !prev.goals }))}
                  style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", borderRadius: 10, border: "1px solid var(--border-color)", background: "var(--bg-soft)", cursor: "pointer", fontSize: 13, fontWeight: 700, color: "var(--text-primary)" }}>
          ...
          {/* Limitations */}
          {(() => {
            const isOpen = expandedSections.limitations;
            return (
              <div>
                <button type="button" onClick={() => setExpandedSections(prev => ({ ...prev, limitations: !prev.limitations }))}
