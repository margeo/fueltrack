...
export default function AiCoach({
  ...
  function buildSystemPrompt(taskType = "general") {
    ...
    const foodPrefsLine = (foodCatStr || allergyStr || cookStr || timeStr)
      ? `\n${isEn ? "FOOD PROFILE" : "ΔΙΑΤΡΟΦΙΚΟ ΠΡΟΦΙΛ"}: ${foodCatStr ? (isEn?"Likes":"Αρέσει")+":"+foodCatStr : ""}${allergyStr ? " | "+(isEn?"Allergies":"Αλλεργίες")+":"+allergyStr : ""}${cookStr ? " | "+(isEn?"Cooking":"Μαγειρική")+":"+cookStr : ""}${timeStr ? " | "+(isEn?"Time":"Χρόνος")+":"+timeStr : ""}${foodCatStr ? "\n"+(isEn?"⚠️ ONLY use foods from the categories listed above. Do NOT suggest foods from categories the user has NOT selected (e.g. if Legumes is not listed, NEVER suggest lentils, beans, chickpeas etc).":"⚠️ Χρησιμοποίησε ΜΟΝΟ φαγητά από τις κατηγορίες που αναφέρονται παραπάνω. ΜΗΝ προτείνεις φαγητά από κατηγορίες που ΔΕΝ επέλεξε ο χρήστης (π.χ. αν δεν αναφέρονται Όσπρια, ΠΟΤΕ μην προτείνεις φακές, ρεβίθια, φασόλια κλπ).") : ""}${allergyStr ? "\n"+(isEn?"⚠️ NEVER suggest foods containing: ":"⚠️ ΠΟΤΕ μην προτείνεις φαγητά που περιέχουν: ")+allergyStr : ""}`
      : "";
