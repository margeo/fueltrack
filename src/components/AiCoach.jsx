...
Δώσε εβδομαδιαίο πρόγραμμα διατροφής 7 ημερών. Στόχος ${targetCalories}kcal ±5%.
Κατανομή: Πρωινό ${Math.round(targetCalories * 0.25)}, Σνακx2 ${Math.round(targetCalories * 0.1)}, Μεσημεριανό ${Math.round(targetCalories * 0.35)}, Βραδινό ${Math.round(targetCalories * 0.2)}kcal.

ΚΑΝΟΝΕΣ ΠΟΙΚΙΛΙΑΣ & ΥΛΙΚΩΝ (μην τους αναφέρεις στην απάντηση):
- Πριν γράψεις το πρόγραμμα, σχεδίασε όλη την εβδομάδα.
- Χρησιμοποίησε ΜΕΓΙΣΤΟ 28 μοναδικά υλικά συνολικά για όλη την εβδομάδα. Μην ξεπερνάς τα 30 σε καμία περίπτωση.
- Βάσισε το πρόγραμμα σε λίγα βασικά υλικά που επαναχρησιμοποιούνται έξυπνα μέσα στην εβδομάδα.
- Το πρόγραμμα πρέπει να έχει ποικιλία, αλλά πρακτική ποικιλία για σούπερ μάρκετ, όχι χαοτική.
- ΜΗΝ βάζεις το ίδιο ακριβώς μεσημεριανό ή το ίδιο ακριβώς βραδινό κάθε μέρα.
- ΜΗΝ επαναλαμβάνεις την ίδια βασική πρωτεΐνη σε όλα τα μεσημεριανά ή σε όλα τα βραδινά.
- Φρόντισε η ποικιλία να είναι κατανεμημένη ισορροπημένα:
  • λίγες βασικές πρωτεΐνες που εναλλάσσονται
  • λίγοι βασικοί υδατάνθρακες/συνοδευτικά που επαναχρησιμοποιούνται
  • λίγα βασικά λαχανικά/φρούτα που εμφανίζονται πολλές φορές
  • απλά snacks που επαναλαμβάνονται λογικά
- ΜΗΝ “σπαταλάς” την ποικιλία σε 15-20 διαφορετικά λαχανικά ή συνοδευτικά.
- ΜΗΝ κάνεις πρόγραμμα τύπου “κάθε μέρα σολομός” ή “κάθε μέρα κοτόπουλο με άλλο λαχανικό”.
- Προτίμησε περίπου:
  • 4-6 βασικές πρωτεΐνες για όλη την εβδομάδα
  • 3-5 βασικούς υδατάνθρακες/συνοδευτικά
  • 6-8 βασικά λαχανικά/φρούτα
  • 3-5 απλά items για πρωινά και snacks
- Κράτα τη λίστα σούπερ μάρκετ ρεαλιστική και σύντομη.
- Χρησιμοποίησε ΤΑ ΙΔΙΑ υλικά σε διαφορετικούς συνδυασμούς.
- ΜΗΝ αναφέρεις τα υλικά ή τον αριθμό τους στην απάντηση.
...
Μετά ρώτα: "Θέλεις να αλλάξω κάτι;"`;
    ...
    if (taskType === "eatnow") return base;
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          text: data.advice,
          msgType: isEatNow ? "eatnow" : undefined
        }
      ]);
      setHasLoaded(true);
    } catch (err) {
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          text: `❌ ${err.message || "Δεν ήταν δυνατή η σύνδεση."}`,
          error: true
        }
      ]);
    } finally {
