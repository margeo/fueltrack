...
- [ ] **Expand rule-based tips + reuse the live state elsewhere** — The Dashboard Macros section now computes three 👉 tips locally (see SummaryTab.jsx `const tips = [];` block) from remainingCalories, macro-share overshoot vs target, and exerciseValue. Worth broadening: (a) add more rules — fasting-window reminders, weekly trend ("you've been under on protein 4 days in a row"), meal-timing hints tied to current time of day, streak-aware nudges, etc.; (b) surface the same computed live-state tips in other parts of the app — Coach tab hero, AI Limit Lock screen, Food tab empty-state, even push-notification copy once we ship notifications. The tip strings live in locales under `summary.tips.*`; moving them to a shared namespace (e.g. `tips.*`) keeps them reusable. Constraint: still no AI round-trip — rule-based means zero latency, zero cost.
